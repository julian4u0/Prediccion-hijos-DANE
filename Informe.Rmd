...
La red se entrena con la función neural_net(X,Y,n_h,learning_rate,epochs, cost_goal = 0, print_cost = False, read = False, save = False), donde 'X' son los datos, 'Y' la variable a predecir, 'n_h' es el numero de capas ocultas, 'cost_goal' es la meta que queremos alcanzar con la función de costo, si 'read' es True indica que la red utilizará los parametros que fueron obtenidos y guardados de anteriores iteraciones, 'save' indica si queremos guardar los parámetros;  los argumentos que se encontraron para rebajar el MSE-training-error fueron los siguientes: 
...
El resultado de nuestro trabajo se puede evidenciar en el aplicativo, nuestro modelo tiene un error de 0.26 de MSE, y un accuracy de 0.74 lo cual significa que  de acuerdo a los parámetros entrados por el usuario, este tiene 74% de probabilidad de obtener una predicción acertada. 
